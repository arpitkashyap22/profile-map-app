...
const ProfileList = ({ profiles }) => {
  ...
  const filteredProfiles = profiles.filter(profile =>
    ...
    (filter.address ? profile.address.toLowerCase().includes(filter.address) : true)
  );
        ...
        {filteredProfiles.length === 0 ? (
          ...
          filteredProfiles.map((profile) => (
            <div key={profile.id} className="bg-gray-700 p-4 rounded-lg flex items-center">
              <img src={profile.photo || defaultPicture} alt={profile.name} className="w-16 h-16 object-cover rounded-full mr-4" />
